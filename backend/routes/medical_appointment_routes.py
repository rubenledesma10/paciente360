...
def buscar_o_crear_paciente(data):
    ...
    if paciente:
        ...
        db.session.commit()
        return paciente, None, False  # ya existía
    ...
    try:
        ...
        return None, ("date_of_birth inválida. Formato esperado YYYY-MM-DD", 400), False
    ...
    return nuevo_paciente, None, True  # recién creado
...
def _crear_turno_validado(data, status_default, is_overbooking=False):
    ...
    paciente, error, es_nuevo = buscar_o_crear_paciente(data)
    if error:
        return {"msg": error[0]}, error[1]

    # CORREGIDO: se excluyen los turnos CANCELADOS. Antes, un turno
    # cancelado seguía "ocupando" el horario para siempre, y ni el
    # paciente ni el administrativo podían volver a usar ese horario.
    ...
    # AGREGADO: mail de bienvenida SOLO si el paciente se creó recién acá
    # (no si ya existía y solo estaba sacando otro turno), y recién
    # DESPUÉS de que todo se guardó bien -> si falla el envío del mail,
    # no rompe la respuesta ni deshace el turno ya creado.
    if es_nuevo:
        try:
...
def update_appointment(appointment_id):
    try:
        ...
        # CORREGIDO: excluir CANCELADO acá también.
        doctor_appointments = MedicalAppointment.query.filter(
...
def reschedule_my_appointment(appointment_id):
    try:
        ...
        # CORREGIDO: excluir CANCELADO acá también.
        doctor_appointments = MedicalAppointment.query.filter(
            MedicalAppointment.id_doctor == appointment.id_doctor,
            MedicalAppointment.date == new_date,
            MedicalAppointment.status != AppointmentStatusEnum.CANCELADO
        ).all()
        if has_overlap(doctor_appointments, new_time, exclude_id=appointment.id_medical_appointment):
