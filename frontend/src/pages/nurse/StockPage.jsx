...
export default function StockPage() {
  // El administrador gestiona el stock (es tarea administrativa) pero no
  // registra el uso de insumos en un paciente: eso es trazabilidad clinica.
  ...
  const loadAll = async () => {
    setLoadError('');
    try {
      const [productsRes, movementsRes, traceRes, patientsRes] =
        await Promise.all([
          getMedicalProducts(),
          getStockMovements(),
          getTraceabilities(),
      ...
      setLoadError(
        'No se pudo cargar la información de stock. Reintentá más tarde.',
      );
    }
  };
  ...
  const expiringProducts = useMemo(
    () => products.filter((p) => p.por_vencer),
    [products],
  );
  const expiredProducts = useMemo(
    () => products.filter((p) => p.vencido),
    [products],
  );
  ...
  const toLocalDayjs = (isoString) =>
    isoString
      ? dayjs(isoString.endsWith('Z') ? isoString : `${isoString}Z`)
      : null;
    ...
    () =>
      movements.filter((m) => {
        const matchesSearch =
          ...
          (m.product_name || '')
            .toLowerCase()
            .includes(movSearch.trim().toLowerCase());
        const matchesType = !movTypeFilter || m.type_movement === movTypeFilter;
        const matchesDate =
          !movSelectedDate ||
          toLocalDayjs(m.date_time)?.isSame(movSelectedDate, 'day');
  ...
  const sortedMovements = useMemo(
    () =>
      [...visibleMovements].sort((a, b) => {
        const diff =
          toLocalDayjs(a.date_time)?.valueOf() -
          toLocalDayjs(b.date_time)?.valueOf();
  ...
  const fefoLotFor = (name) => {
    const candidates = products.filter(
      (p) => p.name_product === name && p.current_stock > 0,
    );
  ...
  const selectedTraceProduct = watchedTraceProductName
    ? fefoLotFor(watchedTraceProductName)
    : null;
  ...
  const openTraceDialog = async () => {
    ...
    try {
      ...
      setTraceError(
        'No se pudo cargar la lista de pacientes atendidos/en espera.',
      );
    }
    setTraceOpen(true);
  };
  ...
  const confirmDeleteProduct = async () => {
    ...
    try {
      ...
      setDeleteProductError(
        err.response?.data?.msg || 'No se pudo dar de baja el producto.',
      );
    } finally {
  ...
  const onMovSubmit = async (values) => {
    ...
    try {
      ...
      setMovError(
        err.response?.data?.msg || 'No se pudo registrar el movimiento.',
      );
    }
  };
  ...
  const onProductSubmit = async (values) => {
    ...
    try {
      ...
      setProductError(
        err.response?.data?.msg || 'No se pudo crear el producto.',
      );
    }
  };
    ...
    <Box>
      ...
      >
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
          <Badge
            badgeContent={lowStock.length + expiringProducts.length}
            color="warning"
          >
            <Button
              variant="outlined"
                ...
                {visibleProducts.map((p) => {
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{
                        backgroundColor:
                          highlightedProductId === p.id_product
                            ? 'action.selected'
                            : undefined,
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip size="small" label={p.type_product || '—'} />
                      </TableCell>
                      <TableCell
                        sx={{
                          color: low ? paletteRaw.danger : 'inherit',
                          fontWeight: 700,
                        }}
                      >
                        {p.current_stock}
                      </TableCell>
                      <TableCell sx={{ color: paletteRaw.gray }}>
      ...
      {tab === 'trz' && (
        <>
          {expiredProducts.length > 0 && (
            <Alert severity="error" sx={{ mb: 2 }}>
              PM vencidos:{' '}
              {expiredProducts.map((p) => p.name_product).join(', ')}
            </Alert>
          )}
          <Box
            sx={{
              ...
              <TableBody>
                {visibleTraceabilities.map((t) => {
                  const patient = patients.find(
                    (p) => p.id_user === t.id_patient,
                  );
                  return (
                    <TableRow key={t.id_traceability}>
                      <TableCell sx={{ fontWeight: 600 }}>
                        {patientName(t.id_patient)}
                      </TableCell>
                      ...
                      <TableCell>
                        <Chip size="small" label={t.batch_number || '—'} />
                      </TableCell>
                      <TableCell>{formatDate(t.expiration_date)}</TableCell>
                      <TableCell sx={{ color: paletteRaw.gray }}>
                        {formatDateTime(t.date_of_use)}
                      </TableCell>
                    </TableRow>
                  );
                })}
      ...
      >
        ...
        <Box
          component="form"
          onSubmit={movForm.handleSubmit(onMovSubmit)}
          noValidate
        >
          <DialogContent
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            ...
            <Button
              type="submit"
              variant="contained"
              disabled={movForm.formState.isSubmitting}
            >
              Aplicar
            </Button>
          </DialogActions>
      ...
      >
        ...
        <Box
          component="form"
          onSubmit={traceForm.handleSubmit(onTraceSubmit)}
          noValidate
        >
          <DialogContent
            sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}
          >
            ...
            {selectedTraceProduct && (
              <Alert severity="info">
                Se descuenta del lote {selectedTraceProduct.batch_number || '—'}{' '}
                (FEFO) · Vence{' '}
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            ...
            <Button
              type="submit"
              variant="contained"
              disabled={traceForm.formState.isSubmitting}
            >
              Registrar
            </Button>
          </DialogActions>
          ...
          >
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <TextField
                ...
                helperText={
                  productForm.formState.errors.minimum_stock_level?.message
                }
              />
            </Box>
          </DialogContent>
            ...
            {lowStock.length === 0 ? (
              ...
              <List dense sx={{ maxHeight: 220, overflow: 'auto' }}>
                {lowStock.map((p) => (
                  <ListItemButton
                    key={p.id_product}
                    onClick={() => goToProduct(p.id_product)}
                  >
                    <ListItemText
      ...
      >
        ...
        >
          ...
          <Typography variant="body2">
            ...
            {deleteProductTarget
              ? ` "${deleteProductTarget.name_product}"`
              : ' este producto'}
            ? Deja de verse en el inventario, pero su historial de movimientos y
            trazabilidad se conserva.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
